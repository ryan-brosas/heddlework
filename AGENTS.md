...
- Mirroring a contribution commit onto `main` must exclude `AGENTS.md` and `.coderabbit.yaml`: the
  contribution tree omits both on purpose, so a plain `git diff main <contribution-head>` carries
  their deletion into the squash merge (observed in PR #35). List the intended paths explicitly, or
