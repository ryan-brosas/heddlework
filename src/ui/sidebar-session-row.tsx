...
/**
 * Enter and Space activate a focusable row, the same rule the app's other keyboard-reachable rows use.
 *
 * The row's own controls (wake, snooze, settle) deliberately keep pointer-only handlers: GPUIX delivers an
 * unconsumed key to the focused element and then to focusable ancestors, so a key handler on a nested
 * control would fire alongside the row's and one press would invoke two actions. The row is the keyboard
 * target; the controls stay pointer affordances.
 */
  ...
  if (lifecycle !== 'active') {
    return (
      ...
      <div testId={lifecycle === 'settled' ? 'sidebar-settled-row' : 'sidebar-snoozed-row'} tabIndex={disabled ? -1 : 0} style={{ height: 36, display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 7, paddingLeft: 10, paddingRight: 6, borderRadius: 7, backgroundColor: colors.sidebar, cursor: disabled ? 'default' : 'pointer', hover: { backgroundColor: colors.sidebarHover } }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} {...(disabled ? {} : { onClick, onKeyDown: activateOnKey(onClick) })}>
        ...
        <div testId="sidebar-wake" tabIndex={disabled ? -1 : 0} style={{ width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: disabled ? 'default' : 'pointer', backgroundColor: hovered ? colors.sidebarHover : colors.sidebar }} {...(disabled ? {} : { onClick: withoutRowClick(onWake) })}>
