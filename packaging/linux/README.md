...
The desktop launch uses `HEDDLEWORK_WORKSPACE` when set, otherwise the last project opened in the app (`$XDG_STATE_HOME/heddlework/workspace`, defaulting under `~/.local/state`), otherwise `$HOME`. The sidebar defaults to that workspace's threads; choose **All projects** to browse every saved session. **Open project** uses the xdg-desktop-portal FileChooser (Hyprland/GNOME portals), then kdialog, then zenity. A portal cancel does not open a second dialog.
