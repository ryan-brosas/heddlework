...
const result = await Bun.build({ ...common, entrypoints: [resolve(root, 'src/web/main.tsx')], outdir, naming: 'main.js', sourcemap: 'linked', tsconfig: resolve(root, 'src/web/tsconfig.json'), define: { ...reactProductionDefine, 'process.platform': '__hwPlatform', 'process.env.HEDDLEWORK_REDUCED_MOTION': 'undefined' }, plugins: [webAliasPlugin(root)], jsx: { runtime: 'automatic', importSource: '@gpuix/react' } })
